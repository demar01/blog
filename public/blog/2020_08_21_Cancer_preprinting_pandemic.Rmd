...
```{r, eval= FALSE}
preprints_all %>% group_by(covid_preprint,posted_date) %>% 
  count() %>% mutate_if(is.character,as.factor) %>% filter(covid_preprint!="other") %>%
   mutate(posted_date = lubridate::ymd(posted_date)) %>% 
    ggplot(aes(x = posted_date, y = n, color = covid_preprint))+
    geom_col(aes(fill = covid_preprint)) +
  scale_x_date(date_labels="%b %y",date_breaks  ="1 month")+
    labs(x = "Posted date (by months)", y = "Preprints deposited") +
  theme(axis.text.x = element_text(angle = 90))
```
![Preprints](https://raw.githubusercontent.com/demar01/blog/master/blogdown/blog/2020_08_21_Cancer_preprinting_pandemic_files/figure-html/cancer_preprint_1-1.png)

Wow! the number of COVID-19 preprints increased tremendously along with the expansion of COVID-19 pandemic. 

Do the cancer-related preprints decreased when the pandemic expanded thought?
```{r, eval=FALSE}
preprints_all %>%
...
![Cancer Preprints](https://raw.githubusercontent.com/demar01/blog/master/blogdown/blog/2020_08_21_Cancer_preprinting_pandemic_files/figure-html/cancer_preprint_2-1.png)

Despite many of the labs being closed (including mine), the number of cancer-related preprints did not decreased during the pandemic. I wonder if any category within biology research was more affected than others `r emo::ji("thinking")`.

There have been a `preprint surge` during the pandemic and as a consequence  preprint servers have enhanced their usual screening procedures, adopting some restrictive publication norms like putting a [bar on manuscripts that are based on computational models.]( https://www.nature.com/articles/d41586-020-01394-6)

I wonder how many of the preprints were later published in peer-review journals after WHO declared the outbreak of COVID-19 pandemic on the 11 of March 2020.
```{r, eval=FALSE}
preprints_all %>%
    mutate(posted_date = lubridate::ymd(posted_date)) %>% 
...
```
![Published Preprints](https://raw.githubusercontent.com/demar01/blog/master/blogdown/blog/2020_08_21_Cancer_preprinting_pandemic_files/figure-html/cancer_preprint_3-1.png)
Almost double the number of covid pre-prints were published in peer-review journals upon preprint publication. This could have to do with the urgency of disseminating scientific knowledge of SAR-CoV2 virus pandemic and the little things known about SAR-CoV2 virus. I wonder if the rate of publication in peer-review journals was affected after biomedical repositories would no longer accept manuscripts making predictions about treatments for COVID-19 solely on the basis of computational work, but I do not know the exact date when that was implemented. 
...
- <b>Comments per preprint:</b> In the [Fraser preprint](https://www.biorxiv.org/content/10.1101/2020.05.22.111294v1.full.pdf) authors found that non-COVID-19 preprints were rarely commented upon, in comparison to COVID-19 preprints (Fig 5D). I wonder if as a consequence of working from home the engagement level of cancer-related preprints increased upon the outbreak of COVID-19 a pandemic on the 11 March 2020 (this could be because scientist that work in this research field would have more office time to read and comment on these preprints).
